...
class MarimoRepository(
    ...
    suspend fun getMarimo(marimoCode: Int): Marimo? {
        return marimoDao.getMarimo(marimoCode).map()
    }

    @RequiresApi(Build.VERSION_CODES.O)
    suspend fun updateMarimo(
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    suspend fun updateMarimo(marimo: Marimo){
    ...
    suspend fun updateWaterMarimo(lastChanged: String, code: Int) {
        marimoDao.updateWater(lastChanged, code)
    }

    suspend fun getAllSync(check: Boolean = false):List<Marimo> {
        if(!check){
    ...
    suspend fun getAverageFrequency():Int {
        var sum = 0
    ...
    suspend fun getMarimoMostFrequentChanged():List<Marimo> {
        return marimoDao.getMarimoMostFrequentChanged().map() { it.map() }
    }

    suspend fun getMarimoLastFrequentChanged():List<Marimo> {
        return marimoDao.getMarimoLastFrequentChanged().map() { it.map() }
    }

    suspend fun addWaterChanges(id: Int, lastWater: String, waterChangeNotes: String?, imagePath: String?
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    suspend fun getHealthScore() : List<MarimoHealthScoreStats> {
    ...
    suspend fun getAllChanges(marimoCode: Int):List<MarimoChange> {
        return marimoDao.getAllChanges(marimoCode).map { it.map() }

    }

    suspend fun addMarimoHealthScore(id: Int, currentDate: String, health: Int) {
    ...
    suspend fun updateHealthScore(marimoCoe: Int, last: String, health: Int) {
        marimoDao.updateHealth(marimoCoe, last, health)
    }

    @RequiresApi(Build.VERSION_CODES.O)
    suspend fun getSpecificHealth(marimoCode: Int, last: String?): Int {
        return marimoDao.getCurrentHealth(marimoCode, last ?: AppUtils.getCurrentDate())
    }

    suspend fun healthExists(marimoCode: Int, last: String) : Int {
    ...
    suspend fun getAllHealth(marimoCode: Int): List<MarimoHealthScore> {
